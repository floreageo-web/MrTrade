...
def backtest_ticker(ticker_symbol: str, period: str = "2y") -> dict | None:
    ...
    try:
        # Folosim yahooquery pentru a evita blocarea yfinance
        ...
        # Curățăm MultiIndex-ul (yahooquery returnează [symbol, date])
        ...
        # Redenumim coloanele pentru a fi compatibile cu librăria ta de indicators.py
        df.rename(columns={
        ...
        # Adăugăm indicatorii (EMA, RSI, ATR) din fișierul tău extern
    ...
    # Începem loop-ul de la un index care permite existența indicatorilor (ex: 210)
    for i in range(210, len(df) - 1):
        row = df.iloc[i]
        prev = df.iloc[i - 1]
        price = float(row["Close"])

        # Verificăm filtrul de preț din config
        ...
        # --- CONDIȚII DE INTRARE ---
        # Extragem valorile calculate de add_indicators
        ...
        # Strategia ta: Trend + RSI + Apropiere de EMA21 + Candle Pattern
...
def _calc_stats(ticker: str, trades: list) -> dict:
    """Calculează performanța matematică a tranzacțiilor."""
    if not trades:
...
def backtest_all(tickers: list = None, top_n: int = 20) -> dict:
    """Rulează backtesting pe toată lista cu pauze între cereri."""
    ...
    for ticker in tickers:
        ...
        # Pauză esențială pentru a nu fi blocați de Yahoo
    ...
    # Sortăm rezultatele după profitabilitate (Total R)
    results.sort(key=lambda x: x.get("total_r", 0), reverse=True)

    # Agregăm datele globale
    ...
    # Salvăm rezultatul într-un JSON pentru referință ulterioară
...
def format_backtest_summary(summary: dict) -> str:
    """Pregătește textul frumos pentru Telegram."""
    if "error" in summary:
