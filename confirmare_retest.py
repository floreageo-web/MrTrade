        ...
        for index, row in df_manual.iterrows():
            ...
            try:
                ...
                if in_zona and rsi_ok and vol_ok and confirmare_wick and inchidere_peste_mijloc:
                    semnale_gasite_acum = True
                    tp_8 = ultima['Close'] * 1.08
                    bot.send_message(CHAT_ID, f"💎 **FINAL PREMIUM SETUP: {ticker_symbol}**\nTarget: {tp_8:.2f}$", parse_mode='Markdown')

            except Exception as e:
                print(f"Eroare {ticker_symbol}: {e}")

    tz_ro = pytz.timezone('Europe/Bucharest')
    ora_ro = datetime.now(tz_ro).hour
    if not semnale_gasite_acum:
        if ora_ro == 22:
            bot.send_message(CHAT_ID, "🌙 **Raport Seară:** Azi nu s-a găsit niciun semnal. Noapte bună! 😴", parse_mode='Markdown')
        else:
            bot.send_message(CHAT_ID, "🔍 **Audit Finalizat:** Sistem activ, nu sunt semnale de intrare momentan. ✅", parse_mode='Markdown')
